...
bool Resolve() {
	static const auto loaded = [] {
		...
		#define LOAD_SYM(name) \
			(reinterpret_cast<void*&>(name) = dlsym(lib, #name)) != nullptr

		return LOAD_SYM(hime_im_client_close)
			&& LOAD_SYM(hime_im_client_focus_in)
			&& LOAD_SYM(hime_im_client_focus_out)
			&& LOAD_SYM(hime_im_client_focus_out2)
			&& LOAD_SYM(hime_im_client_forward_key_press)
			&& LOAD_SYM(hime_im_client_forward_key_release)
			&& LOAD_SYM(hime_im_client_get_preedit)
			&& LOAD_SYM(hime_im_client_open)
			&& LOAD_SYM(hime_im_client_reset)
			&& LOAD_SYM(hime_im_client_set_cursor_location)
			&& LOAD_SYM(hime_im_client_set_flags)
			&& (LOAD_SYM(hime_im_client_set_client_window)
				|| LOAD_SYM(hime_im_client_set_window));
	}();
	return loaded;
}
