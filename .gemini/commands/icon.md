...
2. **IMMEDIATELY** — before any other processing — run this Bash command to save the clipboard image:
   ```bash
   HASH=$(openssl rand -hex 4) && if [[ "$OSTYPE" == darwin* ]]; then bash .gemini/grab_clipboard.sh ".ai/icon_${HASH}.png"; else powershell -ExecutionPolicy Bypass -File .gemini/grab_clipboard.ps1 ".ai/icon_${HASH}.png"; fi
   ```
   On macOS `.gemini/grab_clipboard.sh` is used; on Windows `.gemini/grab_clipboard.ps1`. Both grab the current clipboard image and save it to the specified path.
...
> ```
> gemini mcp add vectosolve --scope user -e VECTOSOLVE_API_KEY=vs_xxx -- npx @vectosolve/mcp
> ```
...
The MCP tool calls the vectosolve API ($0.20/call). The API key is stored in `~/.gemini.json` MCP config (never in the repository).
